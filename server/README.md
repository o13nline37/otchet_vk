...
- `GET /api/spreadsheets` / `POST /api/spreadsheets` — личный (не общий) список Google-таблиц, в которые пользователь успешно выгружал отчёты. Пополняется автоматически после удачного экспорта — см. `js/savedSpreadsheets.js`.
